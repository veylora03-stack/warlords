...
// Season state machine relocated to a dependency-free leaf (Phase 31) so the
// quest engine can resolve the current season without service import cycles.
// Imported for internal use and re-exported for every existing importer —
...
// ── Lifecycle cores (tx-scoped) — state machine lives in season-state.service ─
...
export async function awardSeasonPointsInTx(
  ...
  playerId: string,
  points: number,
  // Phase 32 extends the source catalog with the territory economy.
  source:
    | 'BUILDING_LEVEL_UP'
    | 'UNIT_TRAINED'
    | 'BATTLE_VICTORY'
